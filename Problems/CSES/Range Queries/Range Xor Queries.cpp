...
ll st[MXN * 4 + 5], a[MXN];
int left(int node) { return 2 * n; }
int right(int node) { return 2 * n + 1; }
void calc(int node) { st[node] = st[left(node)] + st[right(node)]; }

void build(int tl = 1, tr = n, int p = 1) {
    if (tl == tr) {
        st[p] = a[tl];
        return;
    }
    int mid = (tl + tr) / 2;
    build(tl, mid, left(p));
    build(mid + 1, tr, right(p));
    calc(p);
}

void modify(int k, int v, int tl = 1, int tr = n, int p = 1) {
    if (tl == tr) {
        st[p] = a[tl];
        return;
    }
    int mid = (tl + tr) / 2;
    if (tl <= k && k <= mid) modify(k, v, tl, mid, left(p));
    else modify(k, v, mid + 1, tr, right(p));
    calc(p);
}

ll query(int ql, int qr, int tl = 1, int tr = n, int p = 1) {
    if (ql <= tr && qr >= tl) {
        return st[p];
    }
    if (qr < tl || ql > tr) {
    ...
    return query(ql, qr, tl, mid, left(p)) + query(ql, qr, mid + 1, tr, right(p));
