...
void build(int tl = 1, int tr = n, int node = 1) {
    if (tl == tr) {
        st[node] = a[tl];
        return;
    }
    int mid = (tl + tr) / 2;
    build(tl, mid, left(node));
    build(mid + 1, tr, right(node));
    calc(node);
}

void update(int k, int v, int tl = 1, int tr = n, int node = 1) {
    if (tl == tr) {
        st[node] = v;
        return;
    }
    int mid = (tl + tr) / 2;
    if (k >= tl && k <= mid) {
        update(k, v, tl, mid, left(node));
    }
    else update(k, v, mid + 1, tr, right(node));
    calc(node);
}

ll query(int ql, int qr, int tl = 1, int tr = n, int node = 1) {
    if (tl >= ql && tr <= qr) {
        return st[node];
    }
    if (tr < ql || tl > qr) {
    ...
    int mid = (tl + tr) / 2;
    return query(ql, qr, tl, mid, left(node)) + query(ql, qr, mid + 1, tr, right(node));
}
